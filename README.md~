...
In case you wish to use a controller class for your application, You must know that `Router` methods second parameter can also be a **controller class name** (which should be as same as controller class file) exsiting in ` /controllers ` directory. Third parameter is controller method you wish to call. Here is an example :
...
to know what is `import()` you can read **EXTRA** part below, in this document. To should just know it adds `SmartStewie` Template engine which is expanding `smarty` and lets you do ` new SmartStewie() `
